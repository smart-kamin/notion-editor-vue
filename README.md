...
Поля документа (`padding-inline: 4rem`) в просмотре **те же**, что в правке,
хотя панели `[+][⠿]`, под которую оставлено место слева, там нет. Это сделано
ради страниц, собранных из просмотра и правки одного документа: с разными
полями текст прыгал бы по горизонтали при переключении режима. Если поля мешают
— перебейте их у себя по классу `.doc-editor__content`.
